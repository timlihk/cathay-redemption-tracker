...
export class CathayClient {
  ...
  private updatePassengers(reqParams: Record<string, string>, adults: number, children: number) {
    // Update common passenger keys when present
    const keys = Object.keys(reqParams);
    for (const k of keys) {
      if (/^ADULT$/i.test(k)) reqParams[k] = String(adults);
      if (/^CHILD$/i.test(k)) reqParams[k] = String(children);
      if (/NB_ADT/i.test(k)) reqParams[k] = String(adults);
      if (/NB_CHD/i.test(k)) reqParams[k] = String(children);
    }
  }

  private async httpAvailability(from: string, to: string, dateYmd: string, adults: number, children: number) {
